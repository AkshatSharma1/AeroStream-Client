...
const FlightSearch = () => {
  ...
  const searchFlights = async (params) => {
    try {
      ...
      if (params.from)
        queryParams.departureAirportId = params.from.toUpperCase(); // Ensure UpperCase (e.g., DEL)
  ...
  const handleAiSearch = async (e) => {
    ...
    try {
      // 1. Send text to backend AI Service
      const aiResponse = await api.post("/ai/parse", {
        query: aiQuery,
      });

      const parsedParams = aiResponse.data.data;
      console.log("🤖 AI Extracted:", parsedParams);

      // 2. Auto-fill the form (Optional UX enhancement)
      setFormData({
        from: parsedParams.departureAirportId || "",
        to: parsedParams.arrivalAirportId || "",
        tripDate: parsedParams.tripDate || "",
        travellers: parsedParams.travellers || 1,
      });

      // 3. Trigger the actual flight search with the extracted data
      // We need to map the JSON keys to what searchFlights expects
      const searchParams = {
        from: parsedParams.departureAirportId,
        to: parsedParams.arrivalAirportId,
        tripDate: parsedParams.tripDate,
        travellers: parsedParams.travellers,
      };

      await searchFlights(searchParams);
    } catch (error) {
      console.error("AI Search Failed:", error);
      alert("AI couldn't understand that query. Please try again.");
    }
  };
  ...
  const handleBook = async (flightId, price) => {
    ...
    try {
      ...
      alert(
        `✅ Booking Successful for ${seatsRequested} passengers! Check your email.`
      );
    } catch (error) {
      alert(
      ...
      <div style={{ marginBottom: "20px", display: "flex", gap: "10px" }}>
        <button
          ...
          style={{
            padding: "10px",
            flex: 1,
            background: searchMode === "normal" ? "#003580" : "#ddd",
            color: searchMode === "normal" ? "white" : "black",
            border: "none",
          }}
        >
          Normal Search
        ...
        <button
          ...
          style={{
            padding: "10px",
            flex: 1,
            background: searchMode === "ai" ? "#7B2CBF" : "#ddd",
            color: searchMode === "ai" ? "white" : "black",
            border: "none",
          }}
        >
          ✨ AI Search
      ...
      {searchMode === "normal" ? (
        ...
        >
          ...
          <p style={{ fontSize: "0.9rem", color: "#555" }}>
            Try: <i>"Flights from DEL to BOM for 2 people on 2024-12-25"</i>
          </p>
          <input
          ...
          >
            <div>
              ...
              <small style={{ color: "green" }}>
                {flight.totalSeats} seats remaining
              </small>
            </div>
